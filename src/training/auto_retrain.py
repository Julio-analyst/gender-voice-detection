...
class AutoRetrainer:
    ...
    def prepare_training_data_from_feedback(self) -> tuple:
        ...
        y = df['actual_label'].map({'Laki-laki': 0, 'Perempuan': 1}).values.astype(np.float32)
        
        print(f"✅ Data prepared:")
        print(f"   X shape: {X.shape}")
        print(f"   y shape: {y.shape}")
        print(f"   Class distribution: Laki-laki={np.sum(y==0)}, Perempuan={np.sum(y==1)}")
        ...
        try:
            ...
            if should_deploy:
                ...
                if old_prod_model.exists():
                    backup_path = Path(f'models/{model_type}_production_backup_{timestamp}.h5')
    ...
    def run_auto_retrain_all_models(self, epochs: int = 20):
        ...
        for model_type, result in results.items():
            if result['success']:
                status = "✅ DEPLOYED" if result['deployed'] else "⚠️  TRAINED (not deployed)"
                accuracy = result['new_accuracy']
                print(f"   {model_type.upper():8s}: {status} (Accuracy: {accuracy:.2%})")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Auto-retrain models based on feedback')
    parser.add_argument('--model', type=str, choices=['lstm', 'rnn', 'gru', 'all'],
                       default='all', help='Model to retrain (default: all)')
    parser.add_argument('--epochs', type=int, default=20,
                       help='Number of training epochs (default: 20)')
    parser.add_argument('--force', action='store_true',
                       help='Force retrain even if threshold not met')
