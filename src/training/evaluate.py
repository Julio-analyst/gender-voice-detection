...
class ModelEvaluator:
    ...
    def plot_confusion_matrix(
        ...
        if save_path is None:
            ...
            save_path = (
                reports_dir / f"confusion_matrix_{self.model_type}_{timestamp}.png"
            )
    ...
    def generate_classification_report(
        ...
        report = classification_report(
            y_true, y_pred_binary, target_names=self.class_names, digits=4
        )
        return report
    ...
    def generate_full_report(
        ...
        cm_path = self.plot_confusion_matrix(
            y_true, y_pred, threshold, save_path=model_dir / "confusion_matrix.png"
        )

        # Plot ROC curve
        print("\n📈 Generating ROC curve...")
        roc_path, auc_score = self.plot_roc_curve(
            y_true, y_pred, save_path=model_dir / "roc_curve.png"
        )
        metrics["auc"] = auc_score

        # Save metrics
        csv_path = self.save_metrics_to_csv(
            metrics, save_path=model_dir / "metrics.csv"
        )

        json_path = self.save_metrics_to_json(
            metrics, save_path=model_dir / "metrics.json"
        )
...
if __name__ == "__main__":
    ...
    y_pred = np.where(
        ...
        np.clip(y_pred - 0.3, 0, 1),
    )  # Lower prob for class 0
