...
class BaseModel:
    ...
    def compile_model(
        ...
        if self.model is None:
            raise ValueError(
                "Model must be built before compilation. Call build() first."
            )
    ...
    def build(self) -> keras.Model:
        ...
        self.model = models.Sequential(
            [
                ...
                # RNN layer
                layers.SimpleRNN(
                    units=self.hidden_units, return_sequences=False, name="rnn_layer"
                ),
                # Regularization
    ...
    def build(self) -> keras.Model:
        ...
        self.model = models.Sequential(
            [
                ...
                # LSTM layer
                layers.LSTM(
                    units=self.hidden_units, return_sequences=False, name="lstm_layer"
                ),
                # Regularization
                layers.Dropout(self.dropout, name="dropout"),
                # Output layer for binary classification
    ...
    def build(self) -> keras.Model:
        ...
        self.model = models.Sequential(
            [
                ...
                # GRU layer
                layers.GRU(
                    units=self.hidden_units, return_sequences=False, name="gru_layer"
                ),
                # Regularization
                layers.Dropout(self.dropout, name="dropout"),
                # Output layer for binary classification
...
def create_model(model_type: str, input_shape: Tuple[int, int], **kwargs) -> BaseModel:
    ...
    model_map = {"rnn": SimpleRNNModel, "lstm": LSTMModel, "gru": GRUModel}

    if model_type not in model_map:
        raise ValueError(
            f"Unknown model type: {model_type}. Must be one of {list(model_map.keys())}"
        )
...
if __name__ == "__main__":
    ...
    for model_type in model_types:
        ...
        try:
            ...
            print(
                f"   Predicted class: {'Perempuan' if prediction[0][0] > 0.5 else 'Laki-laki'}"
            )
