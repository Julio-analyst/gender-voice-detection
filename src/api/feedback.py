...
async def submit_feedback(feedback: FeedbackRequest):
    ...
    try:
        ...
        print(f"   Prediction: {feedback.predicted_label} → Actual: {feedback.actual_label}")
    ...
    try:
        ...
        for model_type in df['model_type'].unique():
            ...
            by_model[model_type] = {
                ...
                'accuracy': float(model_df['is_correct'].mean()) if len(model_df) > 0 else 0.0
            }
...
async def clear_feedback(confirm: str = ""):
    ...
    try:
        ...
        if feedback_file.exists():
            ...
            backup_file = get_feedback_dir() / f"feedback_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            import shutil
            ...
            return {
                "status": "success",
                "message": "No feedback data to clear"
            }
