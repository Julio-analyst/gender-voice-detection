...
class DatasetLoader:
    ...
    def load_from_folders(self, data_dir, male_folder='cowo', female_folder='cewe', 
                         use_segmentation=True, segment_duration=3.0):
        """
            ...
            try:
                ...
                if use_segmentation:
                    segments_data = self._process_audio_with_segments(
                        audio_file, gender='male', counter=male_counter, 
        ...
        for audio_file in tqdm(female_files, desc="Female"):
            try:
                ...
                if use_segmentation:
                    segments_data = self._process_audio_with_segments(
                        audio_file, gender='female', counter=female_counter,
                        segment_duration=segment_duration
                    )
                    for seg_features, seg_metadata in segments_data:
                        X_list.append(seg_features)
    ...
    def _process_audio_with_segments(self, audio_path, gender, counter, segment_duration=3.0):
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load and preprocess gender voice dataset')
    parser.add_argument('--data-dir', type=str, required=True,
                        help='Directory containing male and female folders')
    parser.add_argument('--male-folder', type=str, default='cowo',
                        help='Name of folder with male voices (default: cowo)')
    parser.add_argument('--female-folder', type=str, default='cewe',
